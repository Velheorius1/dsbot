...
// Helper to download a Telegram file by file_id into inbox/
// Clean up inbox files older than 24 hours, every hour
setInterval(() => {
  try {
    ...
    for (const f of files) {
      ...
      try {
        ...
        if (st.mtimeMs < cutoff) rmSync(fp, { force: true })
      } catch {}
    }
  } catch {}
}, 60 * 60 * 1000)
...
bot.on('message:voice', async ctx => {
  ...
  await handleInbound(ctx, ctx.message.caption ?? '(voice message)', undefined, async () => {
    return downloadTgFile(ctx, v.file_id, v.file_unique_id, 'ogg')
  })
})

bot.on('message:audio', async ctx => {
  const a = ctx.message.audio
  await handleInbound(ctx, ctx.message.caption ?? '(audio file)', undefined, async () => {
    return downloadTgFile(ctx, a.file_id, a.file_unique_id, 'mp3')
  })
})

bot.on('message:video_note', async ctx => {
  const vn = ctx.message.video_note
  await handleInbound(ctx, '(video note)', undefined, async () => {
    return downloadTgFile(ctx, vn.file_id, vn.file_unique_id, 'mp4')
  })
})
...
// Catch unhandled errors — prevents crash from killing the entire plugin
bot.catch(err => {
